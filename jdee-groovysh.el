;;; jdee-groovysh.el -- Beanshell JDEE integration.
...
(defcustom jdee-groovysh-separate-buffer nil
  "*Whether or not to use a separate buffer for errors."
  :group 'jdee
  :type 'boolean)


(defclass jdee-groovysh-buffer (bsh-comint-buffer) ()
  "JDEE's Groovysh buffer")

(defmethod initialize-instance ((this jdee-groovysh-buffer) &rest fields)
  (oset this buffer-name "*JDEE Groovysh*")
  (call-next-method))

(defclass jdee-groovysh (bsh)
  ((bsh-cmd-dir      :initarg :bsh-cmd-dir
		     :type string
		     :documentation
		     "Path of the groovysh commmand directory.")
   ...
   (the-bsh        :type jdee-groovysh
		   :allocation :class
		   :documentation
		   "The single instance of the JDEE's GroovyShell."))
  "Class of JDEE BeanShells. There is only one per Emacs session.")

(defmethod initialize-instance ((this jdee-groovysh) &rest fields)
  "Constructor for the JDEE Groovysh instance."
  (call-next-method)
  ...
  (let* ((jdee-java-directory jdee-server-dir))
    ...
    (oset this separate-error-buffer jdee-groovysh-separate-buffer)
    (oset this name "Groovysh")
    (oset-default 'jdee-groovysh the-bsh this)))

(defmethod bsh-create-buffer ((this jdee-groovysh))
  "Creates the JDEE's beanshell buffer."
  (oset this buffer (jdee-groovysh-buffer "JDEE Groovysh buffer")))

(defmethod bsh-build-classpath-argument ((this jdee-groovysh))
  (jdee-build-classpath (oref this cp) 'jdee-global-classpath t))

(defmethod bsh-launch :BEFORE ((this jdee-groovysh) &optional display-buffer)
...
;; Create the Groovysh wrapper object.
(jdee-groovysh "JDEE Groovysh")

(defun jdee-groovysh-running-p ()
  "Returns t if the JDEE's BeanShell instance is running."
  (bsh-running-p (oref-default 'jdee-groovysh the-bsh)))

(defvar jdee-groovysh-read-java-expression-history nil)

(defun jdee-groovysh-read-java-expression ()
  "Read an expression as input guessing initial input at the current point."
  (if mark-active
      (progn
	(setq jdee-groovysh-read-java-expression-history
	      (cons (buffer-substring (region-beginning)
				      (region-end))
		    jdee-groovysh-read-java-expression-history))
	(jdee-groovysh-quote-expr (region-beginning) (region-end) t t))
...
prints out, Emacs has nothing to evaluate or report."
  (interactive (list (jdee-groovysh-read-java-expression)))
  (cl-flet ((log
	     (msg logtype)
	     (when jdee-jeval-debug
	       (with-current-buffer (get-buffer-create "*Groovysh Debug Log*")
		 (goto-char (point-max))
		 (insert (format "%S<" logtype))
		 (insert (if (stringp msg) msg (prin1-to-string msg)))
		 (insert ">")
		 (newline)))))
    (let ((the-bsh (oref-default 'jdee-groovysh the-bsh)))
...
a file in the current directory:
 ...
 (jdee-groovysh-compile-mode-eval \"jde.util.CompileServer.compile(\\\"Test.java\\\");\"
  ...
  (let* ((buffer-obj (bsh-compilation-buffer "buffer"))
	...
	(insert java-expr))
      ...
      (if (not (jdee-groovysh-running-p))
	  (progn
	    (bsh-launch (oref-default 'jdee-groovysh the-bsh))
	    (bsh-eval (oref-default 'jdee-groovysh the-bsh) (jdee-create-prj-values-str))))


      (bsh-buffer-eval
       (oref-default 'jdee-groovysh the-bsh)
       java-expr
       buffer-obj)
...
;;;###autoload
(defun jdee-groovysh-run()
  "*Starts the JDEE version of the BeanShell."
  (interactive)
  (bsh-launch (oref-default 'jdee-groovysh the-bsh) t))

(defun jdee-groovysh-exit ()
  "Closes the existing beanshell process"
  (interactive)
  (if (jdee-groovysh-running-p)
      (let ((process (bsh-get-process (oref-default 'jdee-groovysh the-bsh))))
	(if (and
...
;;;###autoload
(defun jdee-groovysh-quote-expr (&optional start end no-param-p no-quote-wrap-p)
...
(provide 'jdee-groovysh)

;; End of jdee-groovysh.el
